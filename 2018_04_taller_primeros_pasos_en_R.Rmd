...
Si te fijas, nuestro script aún dice Untitle1. Antes de seguir, es importante que lo guardemos. Al hacerlo, aparecerá en la pestaña _Files_.  
...
`babynames` es un paquete que contiene datos sobre nombres registrados en Estados Unidos entre los años 1880 y 2015. El "objeto" que contiene los datos dentro del paquete se llama también `babynames`. No vemos ese objeto en nuestro `Gobal Environment` porque el objeto existe dentro del paquete, pero si hacemos clic sobre Global Environment podemos acceder al "environment" del paquete y ahí lo encontraremos: 


<a href="url"><img src="https://github.com/rivaquiroga/RLadies-Santiago/blob/master/images/entorno_babynames.png" height="200"></a>

En R, podemos ver un objeto simplemente ejecutándolo. Es decir, escribimos el nombre del objeto y ejecutamos el código. De ahora en adelante, en vez de mostrarte un pantallazo del script, el código que haya que ejecutar aparecerá en un recuadro, así:
...
Ahora que conocemos cómo es el conjunto de datos con el que estamos trabajando, podemos empezar a explorarlo y hacernos preguntas. Por ejemplo, podríamos querer saber a cuántas personas le han puesto como Danerys (como el personaje de Juego de Tronos). Para eso podemos utilizar el siguiente código:
...
<a href="url"><img src="https://github.com/rivaquiroga/RLadies-Santiago/blob/master/images/filter_daenerys.png" align="center" height="250"></a>

¡Las primeras Daenerys aparecen en 2012! ¿Y las Aryas? Podemos usar el mismo código, cambiando solo el valor que queremos que tenga la variable nombre: que se igual a Araya. 

```r
babynames %>% 
    filter(name == "Arya")
```
<a href="url"><img src="https://github.com/rivaquiroga/RLadies-Santiago/blob/master/images/filter_arya.png" align="center" height="250"></a>

Hay muchas más Aryas. Y aparecen antes (o sea, por influencia del libro y no de la serie). Y son tantas que no alcanzan a imprimirse en la consola. ¿Cómo podríamos saber cuántas son? 

Antes de responder eso, hablemos un poco del símbolo ` %>% `. ¿Qué es ` %>% `? Este símbolo se llama _pipe_ (en inglés, _tubo_) y en el código lo podemos leer como "luego" o "a continuación". El atajo del teclado para realizarlo es `control` + `shift` + `m` (Linux / Windows) o `command` + `shift` + `m` (Mac.)
...
* tome el objeto `babynames` 
* luego (` %>% `) 
* que filtre (`filter()`) los nombres (`name`) que sean iguales `==` a `Arya` 
* luego (` %>% `)
...
Más arriba comentamos que el Tidyverse era un conjunto de paquetes que compartía un mismo enfoque de trabajo con los datos. Uno de las características de este enfoque tiene que ver con la forma de escribir el código. En particular, con el uso de ` %>% `. Lo que hace este símbolo es "desempaquetar" el código, de modo que lo leamos de arriba hacia abajo y de izquierda a derecha. La manera "tradicional"" de escribir el mismo código sería la siguiente:

```r
sum(babynames$n[babynames$name == "Arya"])
```
Ambos códigos hacen exactamente lo mismo y llegan al mismo resultado (puedes probar ejecutándolo). La diferencia es que el primero que escribimos (usando ` %>% `) resulta más fácil de leer que el segundo, en que tenemos que leer de adentro hacia afuera. 
...
Imagina que nos interesa guardar los datos sobre la cantidad de Aryas porque seguiremos trabajando con ellos. Para no tener que filtrar cada vez, nos conviene crear un nuevo objeto. Eso lo podemos hacer con el símbolo de asignación: `<-`. ¿Cómo funciona? Lo que está a la izquierda del símbolo de asignación es el objeto que queremos crear y lo que está a su derecha el valor que queremos asignarle. Así:

```r
aryas <- babynames %>% 
    filter(name == "Arya")
```
El código crea un nuevo objeto llamado `aryas`, que contiene lo que resulta de aplicar ese filtro a `babynames`. Si ejecutas ese código, no aparecerá nada nuevo en la consola (solo el código que acabas de ejecutar). Sin embargo, en nuestro Global Enviromen (el panel de arriba a la derecha) apareció el objeto que hemos creado:

<a href="url"><img src="https://github.com/rivaquiroga/RLadies-Santiago/blob/master/images/aryas.png" align="center" height="250"></a>
