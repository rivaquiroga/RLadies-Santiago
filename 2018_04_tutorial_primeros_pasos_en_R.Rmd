...
Partamos por el panel de abajo a la derecha. Si te fijas, el panel tiene varias ventanas: 

* __Files__ muestra el directorio (la carpeta) en la que te encuentras actualmente. En mi caso, no hay nada ahí, porque por defecto me muestra el Escritorio (_Desktop_). Es posible que a ti te muestre otra carpeta (por ejemplo, _Documentos_). 
...
* __Plots__ es el lugar donde aparecerán los gráficos que crees. No hemos hecho ninguno por ahora, así que este panel está vacío. 

* __Packages__ muestra la lista de paquetes que tienes instalados en tu computador
...
* __Help__, como su nombre lo indica, es la pestaña en la que podemos encontrar ayuda. Si ingresamos algún término (por ejemplo, el nombre de un paquete o de una función), RStudio nos remitirá a la documentación asociada. 
...
El panel de arriba a la derecha contiene el historial de funciones que hemos ejecutado (_History_), la opción para generar conexiones a bases de datos externas (_Connections_, que no lo utilizaremos por ahora) y _Environment_. Este último panel es muy importante y entender lo que nos muestra es fundamental para comprender cómo funciona R. 
...
* ocupar el atajo de teclado `shift(mayúscula)` + `comando` + `n` (Mac) o `shift(mayúscula)` + `control` + `n`
